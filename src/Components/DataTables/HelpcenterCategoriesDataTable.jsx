...
export default function HelpcenterCategoriesDataTable({ helpcenterCategoriesData, loading, refetch }) {
    ...
    const handleToggleStatus = async (categoryId, currentStatus) => {
        ...
        try {
            await axios.patch(
                `https://api.propxpro.com/api/admin/help-center/categories/${categoryId}/toggle-status`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('userToken')}`
            ...
            toast.success(`Category ${currentStatus ? 'deactivated' : 'activated'} successfully`, { duration: 2000 });
            refetch();
        } catch (error) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await axios.delete(
                `https://api.propxpro.com/api/admin/help-center/categories/${categoryToDelete}`,
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('userToken')}`
                    }
    ...
    const handleAddCategory = async (e) => {
        ...
        try {
            await axios.post(
                'https://api.propxpro.com/api/admin/help-center/categories',
                {
                    name: formData.name,
    ...
    const handleUpdateCategory = async (e) => {
        ...
        try {
            await axios.put(
                `https://api.propxpro.com/api/admin/help-center/categories/${selectedCategory.id}`,
                {
                    name: formData.name,
                    description: formData.description,
                    order: parseInt(formData.order),
    ...
    const filteredCategories = helpcenterCategoriesData?.filter(category => {
        return (
            (filters.global === '' ||
                category.name.toLowerCase().includes(filters.global.toLowerCase()) ||
                category.description.toLowerCase().includes(filters.global.toLowerCase())) &&
            (filters.name === '' ||
            ...
            (filters.status === '' ||
                (filters.status.toLowerCase() === 'active' && category.is_active) ||
                (filters.status.toLowerCase() === 'inactive' && !category.is_active))
        );
    }) || [];
    ...
    return (
        <div className="shadow-2xl rounded-2xl overflow-hidden bg-white">
            ...
            <div className="p-4 border-b flex justify-between items-center gap-4">
                ...
                />
                {currentUser?.data?.data?.permissions?.includes('create_help_category') && (
                    <button
                                ...
                                <tr key={category.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-3 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            {currentUser?.data?.data?.permissions?.includes('edit_help_category') && (
                                                <button
                                            ...
                                            {/* {currentUser?.data?.data?.permissions?.includes('toggle_help_category_status') && (
                                                <button
                                                    className={`${category.is_active ? 'text-red-500 hover:text-red-700' : 'text-green-500 hover:text-green-700'} p-1`}
                                            ...
                                            {currentUser?.data?.data?.permissions?.includes('delete_help_category') && (
                                                <button
